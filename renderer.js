...
ipcRenderer.on('init', (event, data) => {
  ...
  document.getElementById('failoverStratum').value =
    config.failoverStratum || '';
  ...
  gpus.forEach(platform => {
    platform.forEach(gpu => {
      ...
      gpu.hashrateElement = document.getElementById(
        `hashrate${platformID} ${deviceID}`
      );
      gpu.sharesElement = document.getElementById(
        `shares${platformID} ${deviceID}`
      );
      gpu.lastElement = document.getElementById(
        `last${platformID} ${deviceID}`
      );
      gpu.restartsElement = document.getElementById(
        `restarts${platformID} ${deviceID}`
      );
      gpu.manualElement = document.getElementById(
        `manual${platformID} ${deviceID}`
      );
      if (gpu.manualElement) {
        gpu.manualElement.addEventListener('click', function(event) {
          ...
          if (gpu.restartsElement) gpu.restartsElement.innerHTML = '';
        });
      }
...
function addToShares(platformID, deviceID, shares) {
  ...
  let gpu = gpus[platformID][deviceID];
  if (gpu.sharesElement) gpu.sharesElement.innerHTML = shares;
  gpu.shares = shares;
  var totalAccepted = 0,
    totalAccepted2 = 0,
    totalRejected = 0,
...
ipcRenderer.on('hashrate', (event, stats) => {
  ...
  while (gpus[stats.platformID].length <= stats.deviceID)
  ...
  if (stats.shares) addToShares(stats.platformID, stats.deviceID, stats.shares);

  if (
    stats.hashrate &&
    stats.hashrate instanceof String &&
    stats.hashrate.includes('DAG')
  )
    return;
  ...
  if (gpu.manualElement) {
    ...
    switch (nextState) {
      case State.ON:
        gpu.state = State.OFF;
        delete gpu.hashrate;
        if (message && gpu.hashrateElement)
          gpu.hashrateElement.innerHTML = message;
...
function stop() {
  ...
  if (totalHashrateElement) totalHashrateElement.innerHTML = '';
  if (totalSharesElement) totalSharesElement.innerHTML = '';
}
